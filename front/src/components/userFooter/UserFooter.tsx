        ...
        <div className="h-16 pl-2 pr-4 bg-discord-user flex items-center flex-shrink-0">
            <div className="flex w-32 mr-2">
                <div className="flex items-center mr-2">
                    <img onClick={() => changeStatus(user.uid, "offline")} src={user.img_url} className="rounded-full cursor-pointer" width="50px" alt="" />
                </div>
                <div className="flex w-full flex-col truncate">
                    <h2 className="text-white font-bold text-sm truncate">{user.username}</h2>
                    <p className="text-gray-500 text-xs truncate">{(user.sub_text) ? user.sub_text : `#${user.displayNumbers}`}</p>
                </div>
            </div>
